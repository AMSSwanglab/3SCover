...
Sys.setenv(LANGUAGE = "en") #English language
options(stringsAsFactors = FALSE) #chr as factor
```
## input
...
head(df)
col=display.brewer.pal(8, "Spectral")
```
...
ggplot(UCB_lodes,
  ...
  geom_flow(width = 1/3) + #line and suqare width
  geom_stratum(alpha = .9,width = 1/5) + #width and transparency
  geom_text(stat = "stratum", size = 2,color="black") + #size and color of words
  ...
  theme(panel.grid =element_blank()) + #rm grid
  theme(panel.border = element_blank()) + #rm the box
  theme(axis.line = element_blank(),axis.ticks = element_blank(),axis.text = element_blank()) + #rm the axis
  ggtitle("")+
  guides(fill = FALSE) 
